...
**Settings and Logger (v1.8.0):** `Settings` manages plugin-wide configuration in a single `kntnt_ad_attr_settings` option with filter-based defaults: cookie_lifetime, dedup_seconds, enable_logging, log file sizes, and queue retry parameters. `Logger` writes timestamped diagnostic entries to `wp-content/uploads/kntnt-ad-attribution/kntnt-ad-attribution.log`, shared by core and add-on plugins. Log methods: `info(string $prefix, string $message)` and `error(string $prefix, string $message)`. File auto-rotation between configurable max/min KB limits. `Settings_Page` under Settings > Ad Attribution provides UI for log file size limits, cookie, and queue retry settings. Logging toggle and log file management (download, clear) are on the Tools > Ad Attribution page (`Admin_Page`).

**Queue management UI (v1.8.0, updated v1.9.0):** `Queue_List_Table` (WP_List_Table subclass) displays active and recently completed queue jobs with columns: Reporter, Description (label), Created, Last Attempt, Status. Status column shows human-readable messages: "Pending", "Attempt N ready/running/at [time]", "Success after N attempts", "Failed after N attempts". Row actions: "Run Now" (via `Queue_Processor::process_single()`, hidden for done jobs) and "Delete" (via `Queue::delete()`). The `Queue::enqueue()` method accepts optional `label`, per-job `retry_params` (attempts_per_round, retry_delay, max_rounds, round_delay), and `not_before` (Unix timestamp for deferred processing). Conversion reporters return structured items with `payload`, optional `label`, optional `retry_params`, and optional `not_before` keys.

**Deferred queue processing (v1.9.0):** Jobs can specify a `not_before` Unix timestamp via `Queue::enqueue()`. The `dequeue()` method filters out jobs where `not_before` is in the future (NULL = immediately eligible). "Run Now" bypasses `not_before`. The `last_attempt_at` column tracks when each job was last processed, displayed in the admin queue table. `get_next_retry_time()` considers both `retry_after` and `not_before` to schedule the next cron run.

**Click ID structured format (v1.9.0):** `Click_ID_Store::get_for_hashes()` returns structured arrays `['id' => string, 'captured_at' => int]` per platform instead of bare click ID strings. The `captured_at` field is derived from the existing `clicked_at` column. This is a breaking change for consumers of the `kntnt_ad_attr_conversion_reporters` filter that access click IDs.
